...
> User Timelines / Users Extended / Lists / Spaces / Trends のレートは likes / search よりも緩い場合と厳しい場合があるが、
> `x` は保守的に `remaining ≤ 2` (likes/search と同値) で reset まで sleep する (M31 D-6 / M32 D-1 / M33 D-2 / M34)。
> Users Extended / Lists / Spaces / Trends のレートは X API 公式ドキュメントの値 (時期により変動の可能性あり)、実機確認で更新する。
