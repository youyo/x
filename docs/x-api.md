...
### 1.6 Lists エンドポイント (lookup / tweets / members / owned / followed / memberships / pinned、M33)

`x list {get,tweets,members,owned,followed,memberships,pinned}` は X API v2 の Lists 系 7 endpoint を
カバーする。

| エンドポイント | `max_results` | ページネーション | 認証制約 |
|---|---|---|---|
| `GET /2/lists/:id` | — | — | OAuth 1.0a |
| `GET /2/lists/:id/tweets` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a |
| `GET /2/lists/:id/members` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a |
| `GET /2/users/:id/owned_lists` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a |
| `GET /2/users/:id/list_memberships` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a |
| `GET /2/users/:id/followed_lists` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a |
| `GET /2/users/:id/pinned_lists` | — | **— (paginate 非対応)** | **OAuth 1.0a self only** |

CLI 設計上の判断 (M33):

- **`pinned`**: X API 仕様で self only のため、CLI 側で `--user-id` フラグを公開せず毎回 `GetUserMe` で self を解決する (M33 D-7)。
  paginate もサポートされないため `--all` / `--pagination-token` / `--max-results` フラグは登録しない。
- **`owned` / `followed` / `memberships` の `@username`/URL 位置引数**: M32 `extractUserRef` で username 化したあと
  `GetUserByUsername` で ID 解決 → endpoint 呼び出し (2 API call、M33 D-10)。
- **`extractListID`**: 数値 ID または `https://(x|twitter).com/i/lists/<NUM>` URL のみ許可。それ以外は `ErrInvalidArgument` (exit 2、M33 D-6)。
- **paged 全 5 endpoint でクエリパラメータ `pagination_token` を統一**: X API 公式 docs で
  `/2/lists/:id/members` のみ `next_token` 表記が混在するが、実装では他 paged endpoint と
  揃えて `pagination_token` を採用しテストで pin (M33 D-2)。
- **`list tweets` には時刻フィルタなし**: X API は List Tweets endpoint に `start_time` / `end_time` / `since_id` / `until_id`
  をサポートしないため、JST 系フラグも登録しない (M33 D-9)。

公式リファレンス:
- <https://docs.x.com/x-api/lists/list-lookup-by-list-id>
- <https://docs.x.com/x-api/lists/list-tweets/introduction>
- <https://docs.x.com/x-api/lists/list-members/introduction>
- <https://docs.x.com/x-api/users/get-owned-lists>
- <https://docs.x.com/x-api/users/get-list-memberships>
- <https://docs.x.com/x-api/users/get-followed-lists>
- <https://docs.x.com/x-api/users/get-pinned-lists>

### 1.7 Spaces + Trends エンドポイント (M34)

`x space {get,by-ids,search,by-creator,tweets}` および `x trends {get,personal}` は X API v2 の
Spaces 系 5 endpoint と Trends 系 2 endpoint をカバーする。

| エンドポイント | per-call | ページネーション | 認証制約 / 注記 |
|---|---|---|---|
| `GET /2/spaces/:id` | — | — | OAuth 1.0a (アクティブな Space のみ取得可) |
| `GET /2/spaces` | 1..100 IDs | — | OAuth 1.0a |
| `GET /2/spaces/search` | 1..100 (default 100) | **— (X API 仕様、next_token なし)** | OAuth 1.0a |
| `GET /2/spaces/by/creator_ids` | 1..100 IDs | — | OAuth 1.0a |
| `GET /2/spaces/:id/tweets` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a |
| `GET /2/trends/by/woeid/:woeid` | `max_trends` 1..50 (default 20) | — | OAuth 1.0a |
| `GET /2/users/personalized_trends` | — | — | OAuth 1.0a (認証ユーザー固定、`user_id` 引数なし) |

CLI 設計上の判断 (M34):

- **Spaces アクティブのみ**: X API 仕様で live / scheduled のみ取得可能、終了済み Space は不可。
- **`/2/spaces/:id/buyers` を除外**: OAuth 2.0 PKCE 専用のため、本プロジェクトの OAuth 1.0a User Context では呼べない (M34 対象外)。
- **`search` の `--all` 非対応**: `docs.x.com/x-api/spaces/search-spaces` OpenAPI で `meta.next_token` 不在を確認済 (2026-05-15)。
  `SpaceSearchOption` は独立型として確保しており、将来 X API が next_token を追加した場合に
  `WithSpaceSearchNextToken` を後付けできる構造になっている (M34 D-2)。
- **`by-ids` / `by-creator` は `--ids` 必須フラグ**: `cobra.NoArgs` + `MarkFlagRequired("ids")` で
  CSV を受け取る (位置引数だと shell quote が面倒なため、M34 D-5)。
- **`extractSpaceID`**: Space ID は base62 風の英数字混合 (例: `1OdJrXWaPVPGX`)。
  数値限定の List ID とは異なり、`^[A-Za-z0-9]+$` または `https://(x|twitter).com/i/spaces/<ID>` URL のみ許可 (M34 D-15)。
- **Trends パラメータ名差分**: 2 endpoint で X API のクエリ名が異なる:
  - `/2/trends/by/woeid/:id`: **`max_trends`** (≠ `max_results`)、上限 50。`trend.fields` は `trend_name` / `tweet_count`。
  - `/2/users/personalized_trends`: **`personalized_trend.fields`** (≠ `trend.fields`)、`trend_name` / `category` / `post_count` / `trending_since`。
    `max_results` 等のページング/上限指定なし。
  - 上記差分を吸収するため、xapi 層では `TrendWoeidOption` / `TrendPersonalOption` を型分離する (M34 D-4)。
- **`trends personal` は `--user-id` 未公開**: X API 認証ヘッダから自動解決されるため (M34 D-7 / D-17)。
- **Trend DTO は union 構造体**: 両 endpoint が返すフィールドを 1 つの `Trend` 型に集約し、
  未返却フィールドは `omitempty` で省略する (M34 D-3)。

公式リファレンス:
- <https://docs.x.com/x-api/spaces/space-lookup-by-id>
- <https://docs.x.com/x-api/spaces/spaces-lookup-by-ids>
- <https://docs.x.com/x-api/spaces/search-spaces>
- <https://docs.x.com/x-api/spaces/spaces-lookup-by-creator-ids>
- <https://docs.x.com/x-api/spaces/get-tweets-for-a-space>
- <https://docs.x.com/x-api/trends/get-trends-by-woeid>
- <https://docs.x.com/x-api/trends/get-personalized-trends>

### 1.8 Direct Messages Read エンドポイント (M35)

`x dm {list,get,conversation,with}` は X API v2 の Direct Messages 系 4 endpoint をカバーする。

| エンドポイント | per-call | ページネーション | 認証制約 / 注記 |
|---|---|---|---|
| `GET /2/dm_events` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a (DM read permission 必須、認証ユーザー固定) |
| `GET /2/dm_events/:event_id` | — | — | OAuth 1.0a |
| `GET /2/dm_conversations/:dm_conversation_id/dm_events` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a |
| `GET /2/dm_conversations/with/:participant_id/dm_events` | 1..100 (default 100) | `pagination_token` | OAuth 1.0a (1on1 のみ) |

**重要な Tier 制約 (M35 D-13)**:
...
- **Option 型 2 分離**: `DMLookupOption` (single event 用) と `DMPagedOption` (paged 3 endpoint 共通) を分離し、誤用を型レベルで防ぐ (M35 D-1)。
- **`event_types` は CSV クエリ**: X API 仕様で配列パラメータは CSV (`event_types=MessageCreate,ParticipantsJoin`)。CLI 層でホワイトリスト (case-sensitive、`MessageCreate` / `ParticipantsJoin` / `ParticipantsLeave`) を先に検証する (M35 D-3 / D-4 / D-16)。
- **`extractDMEventID`**: DM event ID は `^\d{1,19}$` (X API docs で 19 桁数値固定、M35 D-6)。
- **`extractDMConversationID`**: 緩い regex (`^[A-Za-z0-9_:\-]+$`) で `<userA>-<userB>` (1on1) / `group:<id>` (グループ DM) / 数値 ID の全バリアントを受ける (M35 D-7)。
- **`dm with` の引数解決**: `extractUserRef` を再利用 (M32) し、`@username` / X profile URL を `GetUserByUsername` で ID 解決してから `/dm_conversations/with/<id>/dm_events` を叩く (M35 D-8)。
- **`--user-id` フラグ非公開**: 全 4 endpoint は認証ヘッダから自動でユーザーを解決する (M35 D-14)。
- **`Includes.Media` を新規追加**: `expansions=attachments.media_keys` + `media.fields=...` で X API が返す `includes.media` 配列を silent drop しないよう、`internal/xapi/types.go` の `Includes` 構造体に `Media []Media` を追加 (M35 D-5、advisor 反映)。likes / timeline / list tweets / space tweets 等の他 paged endpoint でも media が取り込めるようになる net improvement。

公式リファレンス:
- <https://docs.x.com/x-api/direct-messages/get-dm-events>
- <https://docs.x.com/x-api/direct-messages/get-dm-event-by-id>
- <https://docs.x.com/x-api/direct-messages/get-dm-events-for-a-dm-conversation>
...
| `GET /2/lists/:id` | 75 req / 15 min | 75 req / 15 min | Free 以上 |
| `GET /2/lists/:id/tweets` | 900 req / 15 min | 900 req / 15 min | Free 以上 |
| `GET /2/lists/:id/members` | 900 req / 15 min | 900 req / 15 min | Free 以上 |
| `GET /2/users/:id/owned_lists` | 15 req / 15 min | 15 req / 15 min | Free 以上 |
| `GET /2/users/:id/list_memberships` | 75 req / 15 min | 75 req / 15 min | Free 以上 |
| `GET /2/users/:id/followed_lists` | 15 req / 15 min | 15 req / 15 min | Free 以上 |
| `GET /2/users/:id/pinned_lists` | 15 req / 15 min | — (OAuth 1.0a 必須) | Free 以上 |
| `GET /2/spaces/:id` | 300 req / 15 min | 300 req / 15 min | Free 以上 (アクティブな Space のみ) |
| `GET /2/spaces` | 300 req / 15 min | 300 req / 15 min | Free 以上 |
| `GET /2/spaces/search` | 300 req / 15 min | 300 req / 15 min | Free 以上 |
| `GET /2/spaces/by/creator_ids` | 300 req / 15 min | 300 req / 15 min | Free 以上 |
| `GET /2/spaces/:id/tweets` | 300 req / 15 min | 300 req / 15 min | Free 以上 |
| `GET /2/trends/by/woeid/:woeid` | 75 req / 15 min | 75 req / 15 min | Free 以上 |
| `GET /2/users/personalized_trends` | 25 req / 24 h / user | — (OAuth 1.0a 必須) | Free 以上 |
| `GET /2/dm_events` | ~1 req / 24 h (Basic) / ~300 req / 15 min (Pro+) | 同左 | **Pro tier 推奨** (Basic は事実上不可、M35 D-13) |
| `GET /2/dm_events/:event_id` | 同上 | 同左 | 同上 |
| `GET /2/dm_conversations/:id/dm_events` | 同上 | 同左 | 同上 |
| `GET /2/dm_conversations/with/:participant_id/dm_events` | 同上 | 同左 | 同上 |

> User Timelines / Users Extended / Lists / Spaces / Trends / DM のレートは likes / search よりも緩い場合と厳しい場合があるが、
> `x` は保守的に `remaining ≤ 2` (likes/search と同値) で reset まで sleep する (M31 D-6 / M32 D-1 / M33 D-2 / M34 / M35)。
> Users Extended / Lists / Spaces / Trends / DM のレートは X API 公式ドキュメントの値 (時期により変動の可能性あり)、実機確認で更新する。
>
> **DM 系の Tier 制約**: Basic tier では DM 系 endpoint のレート制限が極端に厳しく (約 1 req / 24h)、定期実行は現実的でない。**Pro tier ($5,000/月) 以上を推奨**する。詳細は §1.8 を参照。
