...
x dm list                   ※ M35、GET /2/dm_events
  ※ Basic tier (~1 req / 24h) は事実上使用不可。Pro tier ($5,000/月) 以上を推奨
  ※ 取得可能なのは直近 30 日以内のイベントのみ。X App の DM read permission 必要
  ※ --user-id は X API 認証ユーザー固定のため公開しない
  --event-types MessageCreate,ParticipantsJoin,ParticipantsLeave (CSV、case-sensitive)
  --max-results 1..100 / --pagination-token / --all / --max-pages (default 50)
  --no-json / --ndjson (排他) / --dm-event-fields / --expansions / --user-fields / --tweet-fields / --media-fields

x dm get <eventID>          ※ M35、GET /2/dm_events/:event_id
  ※ eventID は 1..19 桁数値のみ
  --dm-event-fields / --expansions / --user-fields / --tweet-fields / --media-fields / --no-json

x dm conversation <conversationID>   ※ M35、GET /2/dm_conversations/:id/dm_events
  ※ conversationID は <userA>-<userB> (1on1) / group:<id> (グループ) / 数値の全バリアントを許容
  (paged 共通フラグ、x dm list と同じ)

x dm with <ID|@username|URL>   ※ M35、GET /2/dm_conversations/with/:participant_id/dm_events
  ※ @username / X URL は GetUserByUsername で ID 解決 (2 API call)
  (paged 共通フラグ、x dm list と同じ)
