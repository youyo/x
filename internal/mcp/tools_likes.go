...
func registerToolLikes(s *mcpserver.MCPServer, client *xapi.Client) {
	tool := gomcp.NewTool(
		...
		gomcp.WithDescription(
			"Fetch liked tweets for a given user (defaults to self). "+
				"Time window can be specified via start_time/end_time (RFC3339 UTC), since_jst (YYYY-MM-DD), "+
				"or yesterday_jst. "+
				"Set all=true to follow next_token and aggregate all results.",
		),
		gomcp.WithString("user_id",
			gomcp.Description("Numeric user ID. Defaults to the authenticated user (self) when omitted."),
		),
		gomcp.WithString("start_time",
			gomcp.Description("Oldest tweet timestamp (RFC3339 UTC). Example: 2026-05-11T15:00:00Z"),
		),
		gomcp.WithString("end_time",
			gomcp.Description("Newest tweet timestamp (RFC3339 UTC). Example: 2026-05-12T14:59:59Z"),
		),
		gomcp.WithString("since_jst",
			gomcp.Description("JST date (YYYY-MM-DD); 00:00-23:59 JST is converted to UTC and overrides start_time/end_time."),
		),
		gomcp.WithBoolean("yesterday_jst",
			gomcp.DefaultBool(false),
			gomcp.Description("If true, sets the time window to yesterday 00:00-23:59 JST. Overrides since_jst, start_time, and end_time."),
		),
		gomcp.WithNumber("max_results",
			gomcp.Min(1), gomcp.Max(100),
			gomcp.DefaultNumber(float64(likedDefaultMaxResults)),
			gomcp.Description("Maximum number of tweets per page (1-100). Default applied when omitted."),
		),
		gomcp.WithBoolean("all",
			gomcp.DefaultBool(false),
			gomcp.Description("If true, follows next_token to fetch all results and aggregates them."),
		),
		gomcp.WithNumber("max_pages",
			gomcp.Min(1),
			gomcp.DefaultNumber(float64(likedDefaultMaxPages)),
			gomcp.Description("Maximum number of pages when all=true. Safety cap to prevent runaway pagination. Default applied when omitted."),
		),
		gomcp.WithArray("tweet_fields",
			gomcp.Description("Values for tweet.fields query parameter (e.g. [\"id\",\"text\"]). Default applied when omitted or empty."),
			gomcp.WithStringItems(),
		),
		gomcp.WithArray("expansions",
			gomcp.Description("Values for expansions query parameter (e.g. [\"author_id\"]). Default applied when omitted or empty."),
			gomcp.WithStringItems(),
		),
		gomcp.WithArray("user_fields",
			gomcp.Description("Values for user.fields query parameter (e.g. [\"username\",\"name\"]). Default applied when omitted or empty."),
			gomcp.WithStringItems(),
		),
	)
