...
| 作成日 | 2026-05-12 |
| 最終更新 | 2026-05-15 (M37-M38 追加キューイング) |
| ステータス | 🚧 Phase I 全完了 (M29-M36) / Phase II 計画中 (M37-M38) |
| スペック | `docs/specs/x-spec.md` (v1.0.0 Approved) |
| マイルストーン総数 | 38 (M37-M38 追加、細粒度方針) |

## Current Focus

M28 までの基盤 28 マイルストーン + Phase I (M29-M36) の全 36 マイルストーンが完了。Phase I は readonly API 包括サポートとして CLI と MCP に readonly エンドポイント約 35 件を追加した。現在 Phase II (M37-M38) を進行中。
...
**Phase II 進行中**:
1. **M37**: GitHub Actions Node.js 24 対応 ← 次の着手対象
2. **M38**: skills/x/SKILL.md 更新 (v0.4.0 対応)
...
#### M31: User Timelines ✅ 完了
- [x] T1: `internal/xapi/timeline.go` 新規 — `GetUserTweets` / `GetUserMentions` / `GetHomeTimeline` + `Each*TimelinePage` (3 関数分離 D-3)
- [x] T2: `internal/cli/timeline.go` 新規 — `timeline tweets` / `timeline mentions` / `timeline home`、新規 `timelineClient` interface (D-12)、3 つ目の aggregator `timelineAggregator` (D-2、M33 で generics 化再評価)
- [x] T3: `internal/cli/root.go` — `AddCommand(newTimelineCmd())` + TestRootHelpShowsTimeline
...
#### M37: GitHub Actions Node.js 24 対応 ⏳ 未着手
- [ ] `.github/workflows/release.yml` の 4 Action をバンプ (docker × 3 + tibdex → actions/create-github-app-token 差し替え)
- [ ] `actionlint` で syntax 確認
- 📄 詳細: [plans/x-m37-nodejs24-actions.md](./x-m37-nodejs24-actions.md)

#### M38: skills/x/SKILL.md を v0.4.0 (M29-M36) に更新 ⏳ 未着手
- [ ] Subcommands テーブルに tweet/timeline/user/list/space/trends/dm の全コマンドを追記
- [ ] Quick Reference に代表例追加、tier 制約明記
- 📄 詳細: [plans/x-m38-skill-update.md](./x-m38-skill-update.md)
...
| 2026-05-15 | 追加 | M37-M38 をキューイング — Node.js 24 対応 (release.yml 4 action バンプ) + skills/x/SKILL.md 更新 (v0.4.0 コマンド群追記) |
